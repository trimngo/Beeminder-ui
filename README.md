...
Goal cards render Beeminder’s `title` as a multiline description and show configured minutes per unit beside the safety information. Fine print is kept off the compact card and is available through the edit dialog. Connected users can edit and save the title through a multiline description editor and Beeminder’s documented goal-update endpoint. The API reference does not list `fineprint` as an update parameter, so the app intentionally presents it as read-only rather than risking a misleading local-only edit.
