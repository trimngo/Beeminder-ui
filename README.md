...
Directly below the summary, a seven-column forecast shows predicted commitment workload from today through the following six days. It projects one Beeminder quantum at a time and multiplies that stable action size by the configured minutes per unit; goals without time metadata do not contribute. A newly entered datapoint therefore cannot resize every future workload block based on recent datapoint history, though Beeminder safety-buffer or road changes can still move the predicted dates. Tap a day to filter the commitment list to goals projected for that day, and tap it again to remove the forecast filter. Search, tags, safe-days, sorting, and **Hide done today** continue to compose with the selected forecast day, and saved views retain that day selection.
