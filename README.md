...
Directly below the summary, a seven-column forecast shows predicted commitment workload from today through the following six days. It uses each goal’s projected action dates and estimated action size, multiplied by the configured minutes per unit; goals without time metadata do not contribute. Tap a day to filter the commitment list to goals projected for that day, and tap it again to remove the forecast filter. Search, tags, safe-days, sorting, and **Hide done today** continue to compose with the selected forecast day, and saved views retain that day selection. The forecast is an estimate and can change with new datapoints, road changes, or safety-buffer updates.
