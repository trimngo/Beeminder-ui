...
Directly below the summary, a seven-column forecast shows predicted commitment workload from today through the following six days. It downloads Beeminder's `fullroad` commitment path and simulates adding one unit on each day where the commitment would otherwise cross that path. This means a goal due today can gain multiple safe days from today's predicted unit instead of automatically being repeated tomorrow. Beeminder's datapoint `quantum` is precision, not session size, and does not create extra forecast sessions. Goals without time metadata do not contribute. Tap a day to filter the commitment list to goals projected for that day, and tap it again to remove the forecast filter. Search, tags, safe-days, sorting, and **Hide done today** continue to compose with the selected forecast day, and saved views retain that day selection.
