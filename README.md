...
The top of the List view shows the estimated time needed to keep currently due commitments safe today. It multiplies each due, unfinished commitment's configured minutes per unit by its Beeminder quantum; due commitments without a time setting are called out below the total. The adjacent lifetime penalty total counts recorded derailments at the app's fixed $5-per-derail estimate.

The same summary bar shows the estimated workload for the commitments currently displayed by the list filters. It totals one Beeminder quantum of work for each visible commitment and calls out visible commitments that do not yet have a time estimate. For example, setting **Safe days ≤** to `3` shows the estimated work represented by commitments within that safety buffer.

Directly below the summary, a seven-column forecast shows predicted commitment workload from today through the following six days. It projects stable one-unit work sessions and multiplies them by the configured minutes per unit; Beeminder's datapoint `quantum` is precision, not session size, and does not create extra forecast sessions. Goals without time metadata do not contribute. A newly entered datapoint cannot resize every future workload block based on recent datapoint history, though Beeminder safety-buffer or road changes can still move the predicted dates. A goal reported as safe for two days will not be forecast for tomorrow. Tap a day to filter the commitment list to goals projected for that day, and tap it again to remove the forecast filter. Search, tags, safe-days, sorting, and **Hide done today** continue to compose with the selected forecast day, and saved views retain that day selection.

After a datapoint is accepted, Bee Today immediately reloads that specific commitment from Beeminder before updating the card and forecast. This makes the card's safety-day count and the forecast use Beeminder's newly calculated safety buffer instead of the value cached before completion.
...
The compact **Export** button in the Commitments heading opens accountability sharing options. It can refresh Beeminder and copy either today’s completed goals with their log messages or a numbered list of every commitment with its slug, tag-free description, and target rate normalized to a weekly or multi-week cadence. Entries without a comment are identified as having no log message. Keeping the action in the heading preserves vertical space for the goal list.
