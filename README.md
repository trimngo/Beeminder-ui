...
Directly below the summary, a seven-column forecast shows predicted commitment workload from today through the following six days. It downloads Beeminder's `fullroad` commitment path and projects stable one-unit work sessions against the rate scheduled for each future road segment. Beeminder's datapoint `quantum` is precision, not session size, and does not create extra forecast sessions. Goals without time metadata do not contribute. A goal reported as safe for two days will not be forecast for tomorrow. Tap a day to filter the commitment list to goals projected for that day, and tap it again to remove the forecast filter. Search, tags, safe-days, sorting, and **Hide done today** continue to compose with the selected forecast day, and saved views retain that day selection.
